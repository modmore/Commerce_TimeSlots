...
{
    protected $classKey = 'ctsScheduleSlot';
    public $key = 'timeslots-scheduleslot-form';
    public $title = '';
    ...
    public function getFields(array $options = array())
    {
        ...
        $fields[] = new NumberField($this->commerce, [
            'name' => 'lead_time',
            'label' => $this->adapter->lexicon('commerce_timeslots.lead_time'),
            'description' => $this->adapter->lexicon('commerce_timeslots.lead_time.desc'),
            'validation' => [
                new Required(),
                new Number(0),
            ],
    ...
    public function getFormAction(array $options = array())
    {
        if (!$this->record->isNew()) {
            return $this->adapter->makeAdminUrl('timeslots/schedule/slot/edit', ['id' => $this->record->get('id')]);
        }
        return $this->adapter->makeAdminUrl('timeslots/schedule/slot/add');
    }
