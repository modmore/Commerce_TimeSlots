...
class Delete extends Page {
    public $key = 'timeslots/schedule/slot/delete';
    ...
    public function setUp()
    {
        ...
        $object = $this->adapter->getObject(\ctsScheduleSlot::class, ['id' => $objectId]);
        ...
        if ($object) {
            $widget = new DeleteFormWidget($this->commerce, [
                'title' => 'commerce_timeslots.delete_slot_named'
            ]);
            $widget->setRecord($object);
            $widget->setClassKey('ctsScheduleSlot');
            $widget->setFormAction($this->adapter->makeAdminUrl('timeslots/schedule/slot/delete', ['id' => $object->get('id')]));
            $widget->setUp();
