...
{
    protected $classKey = 'ctsDate';
    public $key = 'timeslots-planning-form';
    public $title = '';
    ...
    public function getFields(array $options = array())
    {
        ...
        $fields[] = new DescriptionField($this->commerce, [
            'description' => $this->adapter->lexicon('commerce_timeslots.changing_schedule', ['date' => $this->record->get('for_date')]),
            'raw' => true,
        ]);
        ...
        $fields[] = new SelectField($this->commerce, [
            'name' => 'schedule',
            'label' => $this->adapter->lexicon('commerce_timeslots.schedule'),
            'optionsClass' => \ctsSchedule::class,
            'validation' => [
                new Required(),
            ]
        ]);
    ...
    {
        return $this->adapter->makeAdminUrl('timeslots/planning/edit', [
    ...
    public function afterSave()
    {
        ...
        $schedule = $this->adapter->getObject(\ctsSchedule::class, [
        ...
        foreach ($this->adapter->getIterator(ctsDateSlot::class, $c) as $oldSlot) {
        ...
        $c->sortby('time_from');
        $c->sortby('time_until');
        foreach ($this->adapter->getIterator(ctsScheduleSlot::class, $c) as $baseSlot) {
            $newSlot = $this->adapter->newObject(ctsDateSlot::class);
            $newSlot->fromArray([
                'for_date' => $this->record->get('id'),
